...
<table class="table">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{$value->name}}</td>
            <td>
                <form role="form" action="{{route('brand.destroy', $value->id)}}" method="post">
                    @method('DELETE')
            ...
            <td>
                <form action="{{route('brand.update', $value->id)}}" method="POST" enctype="multipart/form-data" class="popup">
                    @method('PUT')
                    @csrf
